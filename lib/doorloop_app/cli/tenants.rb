...
module DoorLoopApp
  module CLI
    class Tenants < Base
      ...
      def list
        data = DoorLoopApp.service.list_tenants(property_id: options[:property], refresh: options[:refresh])
      ...
      method_option :raw,     type: :boolean, default: false, aliases: "-r",
                              desc: "Output raw JSON instead of formatted table"
      def find(query)
        results = DoorLoopApp.service.find_tenant(query, refresh: options[:refresh])

...
module DoorloopMcp
  module CLI
    class Leases < Base
      ...
      def list
        data = DoorloopMcp.service.list_leases(property_id: options[:property], refresh: options[:refresh])
      ...
      def transactions
        ...
        run_with_session do
          ...
          data = DoorloopMcp.service.list_lease_transactions(lease_id: lease_id)
      ...
      def resolve_lease_id_for_tenant(name)
        ...
        results = DoorloopMcp.service.find_tenant(name)

        if results.empty?
          say "Tenant '#{name}' not in DB — refreshing from DoorLoop..."
          results = DoorloopMcp.service.find_tenant(name, refresh: true)
        end

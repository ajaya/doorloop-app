...
class ExecutorTest < Minitest::Test
  ...
  def test_uses_api_layer_first
    ...
    DoorLoopApp::Api::Endpoints::Properties.expects(:new).with(api_client).returns(endpoint_instance)

    executor = DoorLoopApp::Data::Executor.new(
      session: session,
  ...
  def test_falls_back_to_dom_when_api_fails
    ...
    endpoint_instance = mock("properties_endpoint")
    endpoint_instance.expects(:list_all).raises(DoorLoopApp::Api::Error.new("timeout"))
    DoorLoopApp::Api::Endpoints::Properties.expects(:new).with(api_client).returns(endpoint_instance)

    page_instance = mock("properties_page")
    page_instance.expects(:list_all).returns([{ id: "1", name: "Fallback" }])
    DoorLoopApp::Browser::Pages::PropertiesPage.stubs(:new).with(session).returns(page_instance)
  ...
  def test_uses_dom_when_no_api_client
    ...
    executor = DoorLoopApp::Data::Executor.new(session: session)
  ...
  def test_falls_back_to_vision_when_dom_empty
    ...
    vision_result = DoorLoopApp::Vision::VisionResult.new(data: [{ "name" => "Test Prop" }])
  ...
  def test_raises_for_unknown_operation
    executor = DoorLoopApp::Data::Executor.new(session: build_mock_session)
  ...
  def test_stores_properties_in_sqlite
    ...
    store = DoorLoopApp::Store.new(db_path: ":memory:")
    ...
    executor = DoorLoopApp::Data::Executor.new(session: session, store: store)
    result = executor.call(:list_properties)
  ...
  def test_passes_params_to_layers
    ...
    DoorLoopApp::Browser::Pages::UnitsPage.expects(:new).with(session).returns(page_instance)

...
class ApiClientTest < Minitest::Test
  def setup
    @token_store = DoorloopMcp::Api::TokenStore.new(session_dir: Dir.mktmpdir("doorloop_test"))
  ...
  def test_raises_authentication_error_on_401
    ...
    client = build_client(stubs)
    assert_raises(DoorloopMcp::Api::AuthenticationError) do
  ...
  def test_raises_error_on_500
    ...
    client = build_client(stubs)
    assert_raises(DoorloopMcp::Api::Error) do
      client.get("/api/properties")
    end
  end
  ...
  def build_client(stubs)
    ...
    client = DoorloopMcp::Api::Client.new(token_store: @token_store, registry: @registry)
